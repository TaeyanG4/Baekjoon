...
def init(node, start, end):
    ...
    mid = (start + end) // 2
    init(node*2, start, mid) # 왼쪽 자식 노드의 구간곱
    init(node*2 + 1, mid+1, end) # 오른쪽 자식 노드의 구간곱 
    
    # 왼쪽 자식 노드에 있는 구간곱과 오른쪽 자식 노드에 있는 구간곱을 더한 값을 저장
    tree[node] = tree[node*2] * tree[node*2 + 1] % 1000000007

# 세그먼트 트리의 구간곱
# node: 현재 노드
# start: 구하고자 하는 구간곱의 왼쪽 구간
# end: 구하고자 하는 구간곱의 오른쪽 구간
# left: 노드의 왼쪽 구간
# right: 노드의 오른쪽 구간
def seg_sum(node, start, end, left, right):
    # 구하고자 하는 구간곱의 구간 안에 현재 노드의 구간이 포함되지 않는다면
    # 0을 반환한다.
    if end < left or right < start:
        return 1

    # 구하고자 하는 구간곱의 구간 안에 현재 노드의 구간이 포함된다면
    # 현재 노드의 값을 반환한다.
...
# 구간합과 다른점
# 1. 구간합은 덧셈이므로 초기화할 때 0으로 초기화
# 2. 구간합은 덧셈이므로 init, sum, update할 때 덧셈
# 3. 크기가 커지기 떄문에 % 1000000007로 나눠준다.
...
if __name__ == '__main__':
    ...
    for _ in range(m+k):
        ...
        if a == 1:
            ...
            b -= 1
            c -= 1
            print(seg_sum(1, b, c, 0, n-1) % 1000000007)